      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-5">
        ...
        <GlassCard>
          <div className="flex items-center gap-2 mb-4">
            <Zap className="w-4 h-4 text-corridor" />
            <h3 className="font-semibold text-sm">Active Green Corridor</h3>
            <StatusBadge severity="success">Active</StatusBadge>
          </div>
          <div className="relative py-6">
            <div className="absolute top-1/2 left-0 right-0 h-1 bg-gradient-to-r from-success via-success/50 to-success rounded-full -translate-y-1/2 animate-corridor-flow" 
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {signals.map((s, i) => (
              <div key={s.id} className={`p-4 rounded-xl border transition-all animate-fade-in-up ${
                s.mode === "corridor" ? "bg-success/5 border-success/30" :
                s.mode === "emergency" ? "bg-destructive/5 border-destructive/30" :
